...
checkpoint = torch.load('./cnn_classical_PINN-based_train_mode.pth')
...
Qnet_1 = np.array(Qnet.detach().cpu())
ZAdv_1 = np.array(ZAdv.detach().cpu())
MAdv_1 = np.array(MAdv.detach().cpu())
VAdv_1 = np.array(VAdv.detach().cpu())
Tend1 = np.array(Tend.detach().cpu())

Tend_true_1 = np.array(Tend_true.detach().cpu())
