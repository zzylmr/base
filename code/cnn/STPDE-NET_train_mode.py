...
model_weights = './cnn_STPDE-NET_train_mode.pth'
torch.backends.cudnn.enabled = False

model = ConvNet(input_dim=3,hidden_dim=1,kernel_size1=(3,3,3),padding1=(1,1,1),kernel_size2=(1,3,3),padding2=(0,1,1)).cuda()
...
for epoch in range(epochs):
    ...
    for i, data in tqdm(enumerate(validloader)):
        ...
        data1 = data[:,0:3,:,:,:]
        preds1 = model(data1)
        dT = torch.autograd.grad(outputs=preds1, inputs=data1, grad_outputs=torch.ones_like(preds1), retain_graph=True,create_graph=True)[0]
        dT_x = dT[:, 1, :, :, :].reshape(32,1,7,6,27)
        dT_y = dT[:, 2, :, :, :].reshape(32,1,7,6,27)
