...
for epoch in range(epochs):
    ...
    for i, data in tqdm(enumerate(trainloader)):
        ...
        # Ensure label has the same shape as the output
        if len(label.shape) == 4:
            label = label[:, :1, :, :].reshape(-1, 1, 6, 27)
        elif len(label.shape) == 5:
            label = label[:, :1, :, :, :].reshape(-1, 1, 6, 27)
        else:
            raise ValueError(f"Unexpected label tensor shape: {label.shape}")
        ...
        dT_x = dT[:, 0, :, :, :].reshape(n, 1, 7, 6, 27)
        dT_y = dT[:, 1, :, :, :].reshape(n, 1, 7, 6, 27)
        ...
        # Construction data
        Qnet_train = data[:, 3, :, :, :].reshape(n, 1, 7, 6, 27).to(device)
        mld_train = data[:, 4, :, :, :].reshape(n, 1, 7, 6, 27).to(device)
        u_train = data[:, 5, :, :, :].reshape(n, 1, 7, 6, 27).to(device)
        v_train = data[:, 6, :, :, :].reshape(n, 1, 7, 6, 27).to(device)
        u_d_train = data[:, 8, :, :, :].reshape(n, 1, 7, 6, 27).to(device)
        sst1_train = data[:, 0, :, :, :].reshape(n, 1, 7, 6, 27).to(device)
        T_d_train = data[:, 7, :, :, :].reshape(n, 1, 7, 6, 27).to(device)
    ...
    for i, data in tqdm(enumerate(validloader)):
        ...
        pred11 = ((86400 * (Qnet_valid2[:, :, 0, :, :] / (1025 * 4000 * mld_valid2[:, :, 0, :, :])) - (u_valid2[:, :, 0, :, :] * dT_x1) - (v_valid2[:, :, 0, :, :] * dT_y1) - (u_d_valid2[:, :, 0, :, :] * (sst1_valid2[:, :, 0, :, :] - T_d_valid2[:, :, 0, :, :]) / mld_valid2[:, :, 0, :,:])) + sst1_valid2[:, :, 0,:, :])
        pred22 = ((86400 * (Qnet_valid2[:, :, 1, :, :] / (1025 * 4000 * mld_valid2[:, :, 1, :, :])) - (u_valid2[:, :, 1, :, :] * dT_x2) - (v_valid2[:, :, 1, :, :] * dT_y2) - (u_d_valid2[:, :, 1, :, :] * (sst1_valid2[:, :, 1, :, :] - T_d_valid2[:, :, 1, :, :]) / mld_valid2[:, :, 1, :,:])) + sst1_valid2[:, :, 1,:, :])
        pred33 = ((86400 * (Qnet_valid2[:, :, 2, :, :] / (1025 * 4000 * mld_valid2[:, :, 2, :, :])) - (u_valid2[:, :, 2, :, :] * dT_x3) - (v_valid2[:, :, 2, :, :] * dT_y3) - (u_d_valid2[:, :, 2, :, :] * (sst1_valid2[:, :, 2, :, :] - T_d_valid2[:, :, 2, :, :]) / mld_valid2[:, :, 2, :,:])) + sst1_valid2[:, :, 2,:, :])
        pred44 = ((86400 * (Qnet_valid2[:, :, 3, :, :] / (1025 * 4000 * mld_valid2[:, :, 3, :, :])) - (u_valid2[:, :, 3, :, :] * dT_x4) - (v_valid2[:, :, 3, :, :] * dT_y4) - (u_d_valid2[:, :, 3, :, :] * (sst1_valid2[:, :, 3, :, :] - T_d_valid2[:, :, 3, :, :]) / mld_valid2[:, :, 3, :,:])) + sst1_valid2[:, :, 3,:, :])
        pred55 = ((86400 * (Qnet_valid2[:, :, 4, :, :] / (1025 * 4000 * mld_valid2[:, :, 4, :, :])) - (u_valid2[:, :, 4, :, :] * dT_x5) - (v_valid2[:, :, 4, :, :] * dT_y5) - (u_d_valid2[:, :, 4, :, :] * (sst1_valid2[:, :, 4, :, :] - T_d_valid2[:, :, 4, :, :]) / mld_valid2[:, :, 4, :,:])) + sst1_valid2[:, :, 4,:, :])
        pred66 = ((86400 * (Qnet_valid2[:, :, 5, :, :] / (1025 * 4000 * mld_valid2[:, :, 5, :, :])) - (u_valid2[:, :, 5, :, :] * dT_x6) - (v_valid2[:, :, 5, :, :] * dT_y6) - (u_d_valid2[:, :, 5, :, :] * (sst1_valid2[:, :, 5, :, :] - T_d_valid2[:, :, 5, :, :]) / mld_valid2[:, :, 5, :,:])) + sst1_valid2[:, :, 5,:, :])
        pred77 = ((86400 * (Qnet_valid2[:, :, 6, :, :] / (1025 * 4000 * mld_valid2[:, :, 6, :, :])) - (u_valid2[:, :, 6, :, :] * dT_x7) - (v_valid2[:, :, 6, :, :] * dT_y7) - (u_d_valid2[:, :, 6, :, :] * (sst1_valid2[:, :, 6, :, :] - T_d_valid2[:, :, 6, :, :]) / mld_valid2[:, :, 6, :,:])) + sst1_valid2[:, :, 6,:, :])
