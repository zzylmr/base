...
checkpoint = torch.load('./vit_classical_PINN-based_train_modes.pth')
...
for k in range(1):
    for i, data in tqdm(enumerate(testloader)):
        ...
        preds2 = model(data2)
        preds2 = preds2.reshape(-1, 1, 6, 27)
        dT_x = \
        torch.autograd.grad(outputs=preds2, inputs=xx_test, grad_outputs=torch.ones_like(preds2), retain_graph=True,
                            create_graph=True)[0]
        dT_y = \
        torch.autograd.grad(outputs=preds2, inputs=yy_test, grad_outputs=torch.ones_like(preds2), retain_graph=True,
                            create_graph=True)[0]
        aerfa = 0.1
        beta = 0.9
