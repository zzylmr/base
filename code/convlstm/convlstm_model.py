...
class ConvLSTM(nn.Module):
    ...
    def forward(self, input_x, input_frames=7, future_frames=1, output_frames=7,
        ...
        # 确保输出尺寸与标签一致
        outputs = torch.stack(outputs, dim=1)  # 此处可能需要保留所有时间步的输出
        outputs = outputs.permute(0, 2, 1, 3, 4)  # 从 [batch, time, channels, height, width] 调整为 [batch, channels, time, height, width]
